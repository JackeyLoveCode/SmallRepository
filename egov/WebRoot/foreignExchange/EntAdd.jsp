...
<html>
  <head>
    ...
    <script type="text/javascript">
    	function addNewLine(inv_reg_code,investor_name,countryName){
    		...
    		//给每一个单元格设置InnerHtml
    		tableCell1.innerHTML = '<div align="center"><input type="hidden" name="inv_reg_code" value="'+inv_reg_code+'"/>'+investor_name+'</div>';
    		tableCell2.innerHTML = '<div align="center"><input type="hidden" name="countryName" value="'+countryName+'"/>'+countryName+'</div>';
    	...
    	function computeCap(){
			...
			var countryNames = document.getElementsByName("countryName");
			var totalRegCap = 0;
			var outRegCap = 0;
  ...
  <body>
  	<form action="servlet/saveEnt" method="POST" name="EntForm">
	<table>
		<tr><font  color="blue"><center>企业基本信息</center></font></tr>
		<tr>
			<td>组织机构代码<input type="text" name="orgcode" readonly="readonly" value="<%= request.getParameter("orgcode")%>"/></td>
	...
	<table id="Inv_Cap_ProTable">
		...
		<tr>
			...
			<td>注册资本出资额</td>
			<td>利润分配比例</td>
			<td><input type="button" value="查询" style="cursor:hand" onclick="window.open('/egov/foreignExchange/EntList.jsp')"/></td>
		</tr>
	</table>
	<table>
		<tr>
			<td><input type="button" value="确定" style="cursor:hand" onclick="saveEnt()"/></td>
			<td><input type="button" value="返回" style="cursor:hand" onclick="document.location='/egov/foreignExchange/EntView.jsp'"/></td>
		</tr>
	</table>
	</form>
  </body>
