...
<body>
	...
	<tbody>
		...
		<tr>
			<td><a href="/egov/basicInfo/OrgAdd.jsp" target="f3">新增</a></td><td><a href="">清除</a></td>
		...
		<tbody id="dataInvTable">
			...
			<%
			   ...
			   <tr>
			   <td><%=++i %></td>
			  	<td><span style="cursor:hand" onclick="document.location='/egov/servlet/viewInvestor?investor_reg_code='+'<%=inv.getInvestor_reg_code() %>'"><%=inv.getInvestor_reg_code() %></span></td><td><%=inv.getInvestor_name() %></td>
			  	<td><%=inv.getRegdate() %></td><td><%=StringUtil.getStringByCode(inv.getCountry()) %></td><td><%=inv.getUsername() %></td>
			  <%
			  	}
			  	}
			   %>
