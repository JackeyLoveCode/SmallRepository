...
<script type="text/javascript">
	...
	function changePage(pageno){
		document.forms["InvForm"].action = "servlet/pageQueryInv?pageno="+pageno;
		document.forms["InvForm"].submit();
	}
...
<body>
	...
	<tbody>
		...
		<tr>
			...
			<td>登记日期<input type="text" name="startDate" class="tcal" value="${param.startDate }" /></td>
			<td>~<input type="text" name="endDate" class="tcal" value="${param.endDate }"/></td>
		</tr>
		...
		<tbody id="dataInvTable">
		 <c:if test="${not empty page }">
			   ...
			   <tr>
			   <td>${vs.count }</td>
			  	<td><span style="cursor:hand" onclick="window.opener.addNewLine('${inv.investor_reg_code }','${inv.investor_name }','${egov:getStringByCode(inv.country) }')">${inv.investor_reg_code }</span></td><td>${inv.investor_name }</td>
